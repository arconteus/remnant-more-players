...
const actions = {
  ...
  all: ['Format, test and build', async () => {
...
Commands:
  test     Test the runtime helper
  format   Format source and documentation
  check    Check formatting and run tests
  build    Build the PAK and launch helpers
  all      Format, check and build everything
...
async function menu() {
  ...
  const keys = ['all', 'test', 'format', 'check', 'build']
